...
*/
        ...
        function animateItem(flag){
            ...
            $items.eq(curItemIndex+flag).animate(animProperty,250,'ease-in',function(){
                ...
                //$items.eq(curItemIndex).css('-webkit-transform',"scale3d(1,1,1)")
                isAnimate = false;
            });
        }
        ...
        function moveItem(moveItemIndex, newPosition){
            //var perNum=1-Math.abs(step)/distance;
            $items.eq(curItemIndex).css('opacity',1-Math.abs(step)/distance);
            //$items.eq(curItemIndex).css('-webkit-transform',"scale3d("+perNum+","+perNum+",1)");
    ...
    $.fn.touchSlider.defaults = {
        ...
        slidePercent: 0.15,  //拖动超过多少百分比后才翻页
